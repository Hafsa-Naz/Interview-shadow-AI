...
function InterviewSession() {
  ...
  return <div className="min-h-screen bg-slate-950 text-white flex justify-center items-center px-6"><div className="bg-slate-900 w-full max-w-4xl rounded-3xl p-10 shadow-xl">
    <div className="flex justify-between mb-6"><div><h1 className="text-3xl font-bold">AI Mock Interview</h1><p className="text-gray-400">Question {count} of 5 · {question.focus_area}</p></div><p className="text-purple-400 font-semibold">Interview Shadow AI</p></div>
    <div className="w-full bg-gray-700 rounded-full h-3 mb-10"><div className="bg-purple-600 h-3 rounded-full" style={{ width: `${(count / 5) * 100}%` }} /></div>
    ...
    <div className="flex justify-end mt-8"><Button onClick={handleNext}>{loading ? "Saving..." : count === 5 ? "Finish Interview" : "Next Question"}</Button></div>
  </div></div>;
}
